...
type FoodLog = { id: number; foodName: string; cookingMethod: string; amount: number; unit: "grams" | "piece"; grams: number; calories: number; protein: number; carbs: number; fat: number };
  ...
  return <main className="kilo-app h-dvh overflow-hidden bg-[#f4f7f3] text-[#14251e]" onPointerOver={(event: ReactPointerEvent<HTMLElement>) => startIconMotion(event.target, event.relatedTarget, event.currentTarget)} onFocusCapture={(event: ReactFocusEvent<HTMLElement>) => startIconMotion(event.target, event.relatedTarget, event.currentTarget)} onAnimationEnd={finishIconMotion}><div className="mx-auto grid h-full max-w-[1600px] grid-cols-[250px_minmax(0,1fr)_330px]">
    ...
    <section className="min-h-0 overflow-y-auto px-8 py-7">
      ...
      <div className="space-y-4">
        <section className="rounded-3xl border border-[#dbe5dd] bg-white p-5"><div className="mb-4 flex items-center gap-3"><IconBox><UtensilsMotion size={18} /></IconBox><div><h2 className="font-bold">Yemek ekle</h2><p className="text-xs text-[#65746b]">Pişirme ve adet/gram seçimi hesaba yansır.</p></div></div><fieldset disabled={saving} className="food-entry-fields grid grid-cols-[minmax(0,1fr)_120px_85px_90px_auto] gap-2"><div className="relative z-20"><Search className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2 text-[#7b8d82]" size={17} /><input value={query} onChange={(e) => { setQuery(e.target.value); setFood(null); }} placeholder="Besin ara" className="input" style={{ paddingLeft: "2.5rem" }} /><div data-open={suggestions.length > 0 && !food} aria-hidden={!(suggestions.length > 0 && !food)} inert={!(suggestions.length > 0 && !food)} className="dropdown-float absolute z-20 mt-2 w-full rounded-xl border bg-white p-1 shadow-xl">{suggestions.map((x) => <button key={x.id} onClick={() => selectFood(x)} type="button" className="flex w-full justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-[#eef7f0]"><span><b className="block">{x.name}</b><small className="text-[#65746b]">{x.category}</small></span><b className="text-[#216b50]">{x.kcal}</b></button>)}</div></div><select value={method} onChange={(e) => setMethod(e.target.value)} disabled={!food} className="input"><option value="">Yöntem</option>{food?.preparations.map((x) => <option key={x.method}>{x.method}</option>)}</select><select value={unit} onChange={(e) => { const u = e.target.value as "grams" | "piece"; setUnit(u); setAmount(u === "piece" ? "1" : "100"); }} disabled={!food || !portion} className="input"><option value="grams">Gram</option>{portion && <option value="piece">Adet</option>}</select><LabeledInput value={amount} onChange={setAmount} label={unit === "piece" ? "adet" : "g"} /><button onClick={() => void addFood()} type="button" className="rounded-xl bg-[#216b50] px-4 text-sm font-bold text-white"><Plus className="inline" size={16} /> Ekle</button></fieldset>{preparation && <div className="mt-3 flex justify-between rounded-xl bg-[#f1f7f2] px-3 py-2 text-xs text-[#315745]"><b>{unit === "piece" ? `${dec.format(amountNumber)} adet ≈ ${dec.format(equivalentGrams)} g` : `${dec.format(amountNumber)} g`} · {fmt.format(preparation.kcal * equivalentGrams / 100)} kcal</b><span>P {dec.format(preparation.protein * equivalentGrams / 100)}g · K {dec.format(preparation.carbs * equivalentGrams / 100)}g · Y {dec.format(preparation.fat * equivalentGrams / 100)}g</span></div>}</section>
